...
bool Terrain::GenerateHeightMap(ID3D11Device* device, int seed)
{
	bool result;
	...
	for (int j = 0; j<m_terrainHeight; j++)
	{
		for (int i = 0; i<m_terrainWidth; i++)
		{
			...
			m_heightMap[index].x = (float)i;
			m_heightMap[index].y = (float)PN->PerlinNoise2D(i, j, .5f, 32, .13f) * 10; 
			m_heightMap[index].z = (float)j; 
		}
	}
